# Sending logs via the FusionReactor Agent

The FusionReactor agent automatically ships application and error logs to the cloud, and can be configured to scrape additional log files on the server, obfuscate sensitive data, and roll up multiline log entries.

!!! info "This feature is part of OpsPilot"
    This functionality is available in **OpsPilot**, our cloud-native observability platform.
    [View the OpsPilot documentation](https://docs.opspilot.com) or [start a free trial](https://app.opspilot.com).
