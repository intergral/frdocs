...
| `otel.resource.attributes`               | `null`        | `2025.2.0`    | Sets custom OTel resource attributes such as `service.name`, `service.version`, and `deployment.environment`. Example: `service.name=tc91,service.version=1.0.9,deployment.environment=prod`. |
...
| Platform                           | Required Properties                                                                                                                                                                                                                                                                                 | Description                                                                                  |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| **OTel Collector (gRPC)**          | `-Dotel.exporter.otlp.endpoint=http://localhost:4317`<br>`-Dotel.exporter.otlp.protocol=grpc`                                                                                                                                                                                                       | Sends all signals to a local or remote OTel Collector using gRPC.                            |
| **OTel Collector (HTTP/Protobuf)** | `-Dotel.exporter.otlp.endpoint=http://localhost:4318`<br>`-Dotel.exporter.otlp.protocol=http/protobuf`                                                                                                                                                                                              | Sends all signals to an OTel Collector using HTTP/protobuf.                                  |
| **Alloy**                          | `-Dotel.exporter.otlp.endpoint=http://localhost:4317`                                                                                                                                                                                                                                               | Compatible with Alloy OTLP ingestion (gRPC).                                                 |
| **Datadog**                        | `-Dotel.exporter.otlp.endpoint=https://datadoghq.eu`<br>`-Dotel.exporter.otlp.headers=DD-API-KEY=${DD_API_KEY}`                                                                                                                                                                                     | Sends OTLP data to Datadog (region example: EU). Requires API key header.                    |
| **New Relic**                      | `-Dotel.exporter.otlp.endpoint=https://otlp.eu01.nr-data.net:4318`<br>`-Dotel.exporter.otlp.headers=api-key=${NEW_RELIC_KEY}`                                                                                                                                                                       | Sends OTLP signals to New Relic's OTLP ingest endpoint.                                      |
| **Dash0**                          | `-Dotel.exporter.otlp.endpoint=https://ingress.eu-west-1.aws.dash0.com:4317`<br>`-Dotel.exporter.otlp.headers=Authorization=Bearer ${DASH0_TOKEN}`                                                                                                                                                  | Configures shipping to Dash0 using token-based authorization.                                |
| **Grafana Cloud**                  | `-Dotel.exporter.otlp.metrics.endpoint=${GRAFANA_METRICS_ENDPOINT}`<br>`-Dotel.exporter.otlp.traces.endpoint=${GRAFANA_TRACES_ENDPOINT}`<br>`-Dotel.exporter.otlp.log.endpoint=${GRAFANA_LOGS_ENDPOINT}`<br>`-Dotel.exporter.otlp.headers=Authorization=Basic\ ${GRAFANA_USER}:${GRAFANA_PASSWORD}` | Allows separate metric/trace/log endpoints with Basic Auth for Grafana Cloud OTLP ingestion. |
