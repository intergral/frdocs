# Anomaly Detection User Guide

The Anomaly Detection user guide covers how to enable, configure, and tune anomaly detection for your services using the RED (Request, Errors, Duration) framework, including custom detectors built with PromQL.

!!! info "This feature is part of OpsPilot"
    This functionality is available in **OpsPilot**, our cloud-native observability platform.
    [View the OpsPilot documentation](https://docs.opspilot.com) or [start a free trial](https://app.opspilot.com).
