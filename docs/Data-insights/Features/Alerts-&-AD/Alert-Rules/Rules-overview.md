...
### 1. Queries
The **Query** selects the specific dataset you want to monitor (e.g., CPU usage, heap memory, or error rates).

### 2. Conditions
The **Condition** is the threshold your query must reach or exceed to trigger an alert (e.g., `CPU Usage > 90%`).
...
* **Interval:** How often the rule is checked (e.g., every 1 minute).
...
* **Notification routing:** Define where the alert goes (e.g., Slack, Email, or PagerDuty).
